...
namespace Infrastructure.Identity.Services
{
    public class AccountService : IAccountService
    {
        ...
        public async Task ForgotPassword(ForgotPasswordRequest model, string origin)
        {
            ...
            if (account == null) 
                throw new NotFoundException("Invalid email no user found with this email");

            var code = await _userManager.GeneratePasswordResetTokenAsync(account);
            
            var route = "api/account/reset-password";

            var url = string.Concat(route,"?token="+code+"&email="+model.Email);

            var _enpointUri = new Uri(string.Concat($"{origin}/", url));
            var emailRequest = new EmailRequest()
            {
                Body = $"click here to reset your passowrd - {_enpointUri}",
                To = model.Email,
